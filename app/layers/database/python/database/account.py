...
def __get_recursive_childs_by_uuids_query(db: str, account_uuids: list) -> tuple:
    ...
    query = f"""
        WITH RECURSIVE Children(uuid, account_no, name, id, state, is_taxable,
                is_entity_required, post_date, fs_mapping_status, fs_mapping_id, fs_name, account_attribute_id,
                account_type, detail_type, parent_id, fund_entity_id) AS (
            SELECT a.uuid, a.account_no, a.name, a.id, a.state, a.is_taxable,
            a.is_entity_required, a.post_date, a.fs_mapping_status, fs.fs_mapping_id, fs.fs_name,
            att.uuid, att.account_type, att.detail_type, pa.uuid, fe.uuid
            FROM {db}.account a
            LEFT JOIN {db}.fund_entity fe on fe.id = a.fund_entity_id
            ...
            a1.is_entity_required, a1.post_date, a1.fs_mapping_status, fs1.fs_mapping_id, fs1.fs_name,
            att1.uuid, att1.account_type, att1.detail_type, pa1.uuid, fe1.uuid
            FROM {db}.account a1  
            LEFT JOIN {db}.fund_entity fe1 on fe1.id = a1.fund_entity_id
...
def __get_recursive_parents_by_uuids_query(db: str, account_uuids: list) -> tuple:
    ...
    query = f"""
        WITH RECURSIVE Parents(uuid, account_no, name, id, state, is_taxable,
                is_entity_required, post_date, fs_mapping_status, fs_mapping_id, fs_name, account_attribute_id,
                account_type, detail_type, parent_id, parent_inc_id, fund_entity_id) AS (
            SELECT a.uuid, a.account_no, a.name, a.id, a.state, a.is_taxable,
            a.is_entity_required, a.post_date, a.fs_mapping_status, fs.fs_mapping_id, fs.fs_name,
            att.uuid, att.account_type, att.detail_type, pa.uuid, pa.id, fe.uuid
            FROM {db}.account a
            LEFT JOIN {db}.fund_entity fe on fe.id = a.fund_entity_id
            LEFT JOIN {db}.account_attribute att on att.id = a.account_attribute_id
            LEFT JOIN {db}.FS fs on fs.fs_mapping_id = a.fs_mapping_id
            ...
            SELECT a1.uuid, a1.account_no, a1.name, a1.id, a1.state, a1.is_taxable,
            a1.is_entity_required, a1.post_date, a1.fs_mapping_status, fs1.fs_mapping_id, fs1.fs_name,
            att1.uuid, att1.account_type, att1.detail_type, pa1.uuid, pa1.id, fe1.uuid
            FROM {db}.account a1  
            LEFT JOIN {db}.fund_entity fe1 on fe1.id = a1.fund_entity_id
            LEFT JOIN {db}.account_attribute att1 on att1.id = a1.account_attribute_id
            LEFT JOIN {db}.FS fs1 on fs1.fs_mapping_id = a1.fs_mapping_id
