...
from shared.utils import ACCOUNTS_DIR, get_stack_prefix
...
class AccountsBulkStateStack(BaseStack):
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        dead_letter_queue = cdk.aws_sqs.Queue(
            self,
            "ark-sqs-account-bulk-state-process-dl",
            queue_name=self.STACK_PREFIX + ENV["SQS_ACCOUNT_BULK_STATE_PROCESS_DL"],
        )

        queue = cdk.aws_sqs.Queue(
            self,
            id="ark-sqs-account-bulk-state-process",
            queue_name=self.STACK_PREFIX + ENV["SQS_ACCOUNT_BULK_STATE_PROCESS"],
        ...
        lambda_function_state_db = build_lambda_function(
            ...
            description="account bulk state db",
            name="bulk_state_db",
        ...
        qldb_policy = cdk.aws_iam.Policy(
            self,
            get_stack_prefix() + "ark-db-account-bulk-state-qldb-policy",
            policy_name=get_stack_prefix() + "ark-db-account-bulk-state-qldb",
        ...
        cdk.CfnOutput(
            self,
            "ark-account-bulk-state-function-arn",
            value=lambda_function_state_db.function_arn,
            export_name=self.STACK_PREFIX + "ark-account-bulk-state-function-arn",
        )
